...
*   **`gamescript_bridge.py`**: Acts as a bridge between GameScript events and the Python controller. Handles generic events like Chat, IRC messages, and more.
...
*   **`anti_cheat.py`**: Prevents players from building level crossings over railways or roads owned by other companies by delegating checks to the GameScript.
...
*   **`gamescript_connector.py`**: The bridge between Sentinel and the GameScript running inside OpenTTD. It uses `[SENTINEL]` tagged log messages to exchange JSON data.
*   **`statistics_recorder.py`**: Records detailed company statistics (performance, income, bank, loan, cargo, vehicles, infrastructure) from the GameScript into the MySQL database.
