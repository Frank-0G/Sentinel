...
class CityBuilder extends GSController
{
	...
	//function PrepareCB();  //Coragem
	//function PrepareTown(townid); //Coragem
	...
	function StoryStart();
	function SendPopulationN();
	function SendStatisticsN();
	//function SendAdmin(message);
}


function CityBuilder::GetName()			{ return "Novapolis CityBuilder by The_Dude"; }
...
function CityBuilder::StartCBN() {
	...
	if(this.from_save == false){
		...
		//global goal
		local game_goal = GSController.GetSetting("goal");
    local start_year = GSGameSettings.GetValue("starting_year");
    local game_time = GSController.GetSetting("gamelength");
    if(game_time > 0) {
      GSGoal.New(GSCompany.COMPANY_INVALID, GSText(GSText.STR_CITYBUILDER_SCORELIST_LINE, game_time, start_year), GSGoal.GT_NONE, 0);
    } else {
      GSGoal.New(GSCompany.COMPANY_INVALID, GSText(GSText.STR_CITYBUILDER_SCORELIST_LINE, game_goal), GSGoal.GT_NONE, 0);
    }
	...
	/*while (true) {
		this.Process();
		//this.SendPopulationN();
		this.Sleep(sleeptime);
	}*/
}

function CityBuilder::ProcessCBN() {
...
function CityBuilder::CheckEvents(event) {
  ...
  // ### MERGE COMPANY ###
    ...
    local oldid = merge.GetOldCompanyID();
    SendAdmin( { event = "citybuilder", action = "unclaimed", company = oldid } );  // still on tests
  ...
  // ### NEW COMPANY ###
	if(eventType == GSEvent.ET_COMPANY_NEW)
	{
		//new company
    local game_goal = GSController.GetSetting("goal");
    local max_claim = GSController.GetSetting("claimpop");
    local game_time = GSController.GetSetting("gamelength");
    ...
    if(this.log == 0) {
      ...
      if(GSController.GetSetting("gamelength") == 0) GSGoal.Question(0, cid, GSText(GSText.STR_WELCOME, max_claim, game_goal), GSGoal.QT_INFORMATION, GSGoal.BUTTON_OK);
      else GSGoal.Question(0, cid, GSText(GSText.STR_WELCOMET, max_claim, game_time), GSGoal.QT_INFORMATION, GSGoal.BUTTON_OK);
    }
...
function CityBuilder::DailyLoop(){
  ...
  foreach(company in this.companies) {
    local populationMessage = { event = "populationupdated", company = company.id, population = GSTown.GetPopulation(company.town_id) };
    SendAdmin(populationMessage);
  }
}

function CityBuilder::MonthlyLoop(){
	local goal_id;
...
function CityBuilder::YearlyLoop(){
...
function CityBuilder::HQClaimTown(){
	//HQ claims
	foreach(company in this.companies) {
		...
		if(company.hq != hq_tile) { //check for change
			...
			if(GSTown.IsCity(closest_town_id)){
				...
				GSNews.Create(GSNews.NT_GENERAL, GSText(GSText.STR_CITY_NOTCLAIMED, company.id), GSCompany.COMPANY_INVALID, GSNews.NR_NONE, 0);
			...
			/* if town has bigger population than allowed to claim */
				...
				if(company.town_id != INVALID_TOWN) this.TownRemoveByID(company.town_id); //unclaim old first if any
				company.town_id = INVALID_TOWN;
				GSNews.Create(GSNews.NT_GENERAL, GSText(GSText.STR_TOWN_POPLIMIT, company.id, closest_town_id), GSCompany.COMPANY_INVALID, GSNews.NR_NONE, 0);
			...
			/* if town is already owned, send warning */
				...
				if(company.town_id != INVALID_TOWN) this.TownRemoveByID(company.town_id); //unclaim old first if any
				company.town_id = INVALID_TOWN;
				GSNews.Create(GSNews.NT_GENERAL, GSText(GSText.STR_TOWN_HAS_OWNER, closest_town_id, company.id), GSCompany.COMPANY_INVALID, GSNews.NR_NONE, 0);
			...
			/* if town is too far away, send warning */
				...
				if(company.town_id != INVALID_TOWN) this.TownRemoveByID(company.town_id); //unclaim old first if any
				company.town_id = INVALID_TOWN;
				GSNews.Create(GSNews.NT_GENERAL, GSText(GSText.STR_TOWN_NOTCLAIMED, company.id), GSCompany.COMPANY_INVALID, GSNews.NR_NONE, 0);
			...
			/* There is somebody in neighbourhood */
				...
				if(company.town_id != INVALID_TOWN) this.TownRemoveByID(company.town_id); //unclaim old first if any
				company.town_id = INVALID_TOWN;
				GSNews.Create(GSNews.NT_GENERAL, GSText(GSText.STR_TOWN_NEIGHBOUR_CLOSE, company.id), GSCompany.COMPANY_INVALID, GSNews.NR_NONE, 0);
			...
			/* if town is close enough (30), claim it */
				...
				if(this.townarea > 0){
          local townname = GSTown.GetName(company.town_id);
				  local town_location = GSTown.GetLocation(company.town_id);
				  SendAdmin( { event = "citybuilder", action = "claimed", company = company.id, town = townname, population = GSTown.GetPopulation(company.town_id), x = GSMap.GetTileX(GSTown.GetLocation(company.town_id)), y = GSMap.GetTileY(GSTown.GetLocation(company.town_id)) } );
				...
				GSNews.Create(GSNews.NT_GENERAL, GSText(GSText.STR_TOWN_CLAIMED, company.id, company.town_id), GSCompany.COMPANY_INVALID, GSNews.NR_TOWN, company.town_id);
				local populationMessage = { event = "populationupdated", company = company.id, population = GSTown.GetPopulation(company.town_id) };
        SendAdmin(populationMessage);
...
function CityBuilder::ChangeTownString(){
	local townpop = 0;
...
function CityBuilder::GetCompanyByID(id){
	foreach(company in this.companies) {
...
function CityBuilder::CompanyRemoveByID(companyid){
...
function CityBuilder::GetTownByID(townid){
	foreach(town in this.townlistCB){
...
function CityBuilder::TownRemoveByID(townid){
	for(local i = 0, size = this.townlistCB.len(); i < size; i++) {
		if(this.townlistCB[i].id == townid){
			...
			if(this.townarea){
        SendAdmin( { event = "citybuilder", action = "unclaimed", company = this.townlistCB[i].owner } );
...
function CityBuilder::TownHasOwner(townid){
	foreach(company in this.companies) {
...
function CityBuilder::ClaimedTownNearby(townid, companyid){
...
function CityBuilder::CargoGetNext(population){
...
/* load cargo settings */
function CityBuilder::PrepareCB(){
	...
	this.townarea = GSController.GetSetting("townarean");
...
function CityBuilder::StartTownMonitor(companyid, townid){
	foreach(cargo in this.CBcargo) {
...
function CityBuilder::StopTownMonitor(companyid, townid){
	foreach(cargo in this.CBcargo) {
		if (companyid != GSCompany.COMPANY_INVALID && GSTown.IsValidTown(townid)) {
...
function CityBuilder::LimitCityPopulation(){
...
function CityBuilder::TownsUpdate(goal_gui_update){
	foreach(company in this.companies) {
...
*/
function CityBuilder::TownUpdate(companyid, townid, update){
		...
		if(update){
			...
			if(townpop >= cargo.from){
				if(this.max_storage == 0 || cargo.decay == 100){
					...
					//txt = GSText(GSText.STR_TOWN_REQ_GUI, 1 << cargo.id, town.storage[cargo.id], 100*town.storage[cargo.id]/(this.max_storage * req + 1), town.delivered[cargo.id], req);
					txt = GSText(GSText.STR_TOWN_REQ_GUI, 1 << cargo.id, town.delivered[cargo.id], req, town.storage[cargo.id], 100*town.storage[cargo.id]/(this.max_storage * req + 1));
				}
  ...
  // Changed <<-- Coragem
	if(cargoSize != TOWNGUI_LIMIT){
	  ...
	  else if(town.growing) txt = GSText.STR_TOWNW_GROWTYPE_GROW; // Original
	  //else if(town.growing) txt = GSText.STR_TOWNW_GROWTYPE_LIGHT_GROW;
		else txt = GSText.STR_EMPTY0; //GSText.STR_TOWNW_GROWTYPE_NOTGROW;  // Coragem
	}
	else{
...
function CityBuilder::SendPopulationN()
{
  foreach(company in this.companies) {
    local populationMessage = { event = "populationupdated", company = company.id, population = GSTown.GetPopulation(company.town_id) };
    SendAdmin(populationMessage);
...
function CityBuilder::SendStatisticsN(cargo_suffix_fnc)
{
  ...
  local townid = -1;
  local town, townpop, cargo_goal;

  foreach(company in this.companies)
  {
		townid = company.town_id;
		town = this.GetTownByID(townid);
    
    //Sentinel.IrcPublicMessage("TownID: " + townid + ".");
		if((townid >= 0) && (townid != 65535))
			{
			
			// Now, let's fill the table / dictionary again with all statistics and demands for ALL claimed towns!
			//SendAdmin( { event = "citybuilder", action = "townstats", company = cm, townid = townid, townname = GSTown.GetName(townid), population = GSTown.GetPopulation(townid), housecount = GSTown.GetHouseCount(townid), growthrate = GSTown.GetGrowthRate(townid), statue = GSTown.HasStatue(townid), location = "" + GSMap.GetTileX(GSTown.GetLocation(townid)) + "x" + GSMap.GetTileY(GSTown.GetLocation(townid)), companyrating = GSTown.GetRating(townid,cm) } );
			SendAdmin( { event = "citybuilder", action = "townstats", company = company.id, townid = townid, townname = GSTown.GetName(townid), population = GSTown.GetPopulation(townid), housecount = GSTown.GetHouseCount(townid), growthrate = GSTown.GetGrowthRate(townid), statue = GSTown.HasStatue(townid), location = "" + GSMap.GetTileX(GSTown.GetLocation(townid)) + "x" + GSMap.GetTileY(GSTown.GetLocation(townid)) } );

      // ** NEEDS fix after here
			//for(local i = 0; i < 32; ++i)
			foreach(cargo in this.CBcargo) {
				{
				townpop = GSTown.GetPopulation(townid);
				cargo_goal = townpop * cargo.req / 1000;	//current requirements
				if(townpop >= cargo.from)
					{
					//has_goal = true;
					local cargo_supply = (town.delivered[cargo.id] + town.storage[cargo.id]);
					local cargo_stocked = town.storage[cargo.id];

					if( cargo.decay != 100)
						{			
						SendAdmin( { event = "citybuilder", action = "towndemands", townid = townid, cargo_suffix = cargo_suffix_fnc(cargo.id), cargo_supply = cargo_supply, cargo_goal = cargo_goal, cargo_stocked = cargo_stocked } );	
						}
					else
						{		
						SendAdmin( { event = "citybuilder", action = "towndemands", townid = townid, cargo_suffix = cargo_suffix_fnc(cargo.id), cargo_supply = cargo_supply, cargo_goal = cargo_goal, cargo_stocked = 0 } );	
						}
					}
...
require("n_minor.nut");
require("n_saveload.nut");
require("n_goalprogress.nut");
