// Dependencies loaded by wrapper.nut
...
function CityBuilder::SendAdmin(message)
{
	...
	{
		Sentinel.Error("Error sending to admin interface!");
	}
}

function CityBuilder::OnAdminEvent(data)
{
    ...
    if (data.event == "hqbuilt") {
        ...
        if ("tile" in data) {
            ...
            local cid = ("company" in data) ? data.company.tointeger() : 255;
            {
