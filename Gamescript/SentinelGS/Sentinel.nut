...
class Sentinel
{
    ...
    static function _ShouldLog()
    {
        try {
            local val = GSController.GetSetting("log_level");
            // Level 0: No logs, Level 1: All logs
            if (val != null && val.tointeger() == 0) return false;
        } catch(e) { }
        return true;
    }
    ...
    static function InfoMessage(message) { Sentinel.Log(message); }
    static function DebugMessage(message) { Sentinel.Debug(message); }
    static function ServerChat(message) { Sentinel.ChatPublic(message); }
    static function TeamChat(message, company_id) { Sentinel.ChatTeam(company_id, message); }
    static function GetCargoName(cargo_id) { return Sentinel.GetCargoSuffix(cargo_id); }
    static function IrcPublicMessage(message) { Sentinel.IrcMsg(message); }
